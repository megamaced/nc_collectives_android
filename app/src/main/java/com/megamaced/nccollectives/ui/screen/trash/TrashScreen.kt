...
internal fun TrashScreen(
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                title = { Text("Trash", style = MaterialTheme.typography.titleMedium) },
                navigationIcon = {
                ...
                ui.items.isEmpty() ->
                    ...
                    LazyColumn(modifier = Modifier.fillMaxSize()) {
                        items(ui.items, key = { it.id }) { page ->
                            TrashRow(
                                page = page,
                                onRestore = { viewModel.restore(page.id) },
                                onPurge = { pendingPurge = page },
                            )
                            HorizontalDivider()
                        }
                    }
    ...
    pendingPurge?.let { page ->
        AlertDialog(
            onDismissRequest = { pendingPurge = null },
            title = { Text("Permanently delete?") },
            text = {
                Text("\"${page.title}\" will be deleted forever. This can't be undone.")
            },
            confirmButton = {
                TextButton(onClick = {
                    viewModel.purge(page.id)
                    pendingPurge = null
                }) { Text("Delete forever") }
            },
            dismissButton = {
...
private fun TrashRow(
    ...
    Row(
        ...
        Column(modifier = Modifier.weight(1f)) {
            ...
            if (page.lastUserDisplayName.isNotEmpty()) {
                Text(
                    text = "By ${page.lastUserDisplayName}",
